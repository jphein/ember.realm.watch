...
INLINE_SVG = {
    ...
    # Generated by enclosure/tools/make_mobile_renders.py, which slices and projects
    # ember_mobile_case.py directly. Inlined for the same two reasons as the desk figures:
    # `currentColor` so one drawing is correct in both themes, and named group ids the
    # stylesheet reaches inside to pick a feature out of a busy section.
    #
    # The three detail sections earn their place by showing what no other figure can. The
    # vent one is the load-bearing example: "no straight path across this wall" is a claim
    # in prose and a boolean in the build, and it is a THING YOU CAN SEE only here.
    "mobile_exploded":    Path(__file__).resolve().parent / "renders" / "mobile-exploded.svg",
    "mobile_cross":       Path(__file__).resolve().parent / "renders" / "mobile-cross.svg",
...
COPY = {
    ...
    # The two mobile shaded views. Raster is right for exactly these: they are
    # photograph-like rather than line art, and nothing about them needs to change with
    # the theme -- the opposite of the reason the sections above are inline SVG.
    "mobile_hero":          Path(__file__).resolve().parent / "renders" / "mobile-hero.png",
    "mobile_cover":         Path(__file__).resolve().parent / "renders" / "mobile-cover.png",
