...
def main() -> int:
    ...
    # A self-contained page must reference nothing off-box. The SVG favicon's xmlns is
    # an identifier, not a fetch, and is the one legitimate absolute URL.
    for bad in ("http://", "https://", "//cdn", "fonts.googleapis"):
        hits = [m.start() for m in re.finditer(re.escape(bad), html)
                if "w3.org" not in html[max(0, m.start() - 40):m.start() + 40]]
        if hits:
            print(f"  !! WARNING: {len(hits)} reference(s) to {bad!r}")
    return 0
