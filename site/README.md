...
build.py                 ← then run this (it also runs build_print_sheet.py)
build_print_sheet.py     enclosure/PRINT-SHEET.md → docs/print-sheet.html
check_generated_current.py   pre-commit: source staged without its rebuilt page
check_served_current.py      after a push: what a VISITOR downloads vs origin/main
renders/                 enclosure figures — GENERATED, not hand-drawn:
                           case-*    enclosure/tools/make_renders.py        (desk)
                           mobile-*  enclosure/tools/make_mobile_renders.py (battery)
...
docs/                    ← the deliverable; stage this whole directory
  ...
  print-sheet.html    generated; .nojekyll would otherwise serve the .md as text
...
**The figures under `renders/` carry no hand-drawn geometry, and that is load-bearing.** The
button outline was once typed twice — once as a solid in `ember_case.py`, once as a polygon in
`make_renders.py` — and the two copies were free to disagree forever with nothing to notice.
They did: both were rectangles, the part was asked to become hexagons, and only the figure is
ever looked at. Every figure now reads the case module. *A second hand-drawn copy of geometry
is not documentation of it, it is a rumour about it.*
...
## Editing a source is not publishing it, and publishing is not serving

Three separate gaps, and each one has produced a wrong **published** page while the repo looked
correct. The two checks above exist because notes about this did not work:

| gap | symptom seen | caught by |
|---|---|---|
| source edited, page not rebuilt | `docs/print-sheet.html` said **"⛔ DO NOT PRINT"** for hours after the source was cleared | `check_generated_current.py`, pre-commit |
| committed, not pushed | the retraction sat locally while the page still warned | `git status` / the sweep's ahead-behind line |
| pushed, not yet served | the fixed STL download served the **blocked** part for ~1 min | `check_served_current.py`, after a push |

⚠️ **`enclosure/PRINT-SHEET.md` does not look like a build input.** It is a markdown file in the
enclosure directory, and nothing at the point of editing says a page depends on it — which is
why the guard is mechanical rather than a request to remember. If you add another
source → page pair, add it to `PAIRS` in `check_generated_current.py` in the same change, and
run `--self-test`.

**`renders/` is now in `PAIRS` too, and it should have been from the start.** The guard's own
list of what it exists to prevent opens with *"`docs/assets/case-hero.png` was one render behind
after the case geometry changed"* — and the figures were never actually added, so that first
entry was the one case still uncovered. It matters more than "cosmetic" suggests: `build.py`
**inlines** the SVGs into `docs/index.html`, so regenerating a section drawing and staging it
without rebuilding publishes the previous geometry while the repo looks correct. All 13 figures
are listed, and `_compare` runs each builder once regardless of how many are staged, so the
guard did not get 13× slower. `--self-test` reports **15 pairs**.

The hook is not tracked by git (`.git/hooks` never is), so **a fresh clone starts unprotected**.
The install line is in [`docs/verification.md`](../docs/verification.md).
