...
| `GRILLE_STYLE` | `"hex"` or `"ridge"`. Both solved to the same **673 mm² nominal** open area, so it is aesthetic rather than acoustic. Measured on the hex field (`tools/grille_area.py`): **throat 640.8 mm² across 53 apertures** (−4.8 % on the solve — see below), **mouth 779.9 mm² across 53 apertures**, and a **946.6 mm² field** (38 × 25, r2). The nominal figure describes the throat, which is the restriction that governs level. The throat now sits **under** the solve because the finer lattice spends open area on webs — a level decision, not a defect |
