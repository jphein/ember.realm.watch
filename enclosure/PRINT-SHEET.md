...
|---|---|---|---|---|
| `ember-front-bezel.stl` | 1 | **front face DOWN** on the bed — ✅ **the STL is now exported already in this orientation, so load and print; no flip needed** (issue #25: it used to export face-UP, standing the part on four ⌀5.40 boss tips totalling 71.9 mm² with the whole 1847.9 mm² face in the air) | **none** | ⛔ **REPRINT — this part changed after JP printed one, again in depth only. The front-face deboss went 0.80 → 0.96 mm (#48): JP's verdict on the printed 0.80 bezel was "it's good but go even deeper next time pls".** (0.48 → 0.80 was #34, same story one rev earlier.) Sizes did **not** change — `BEZ_AFLAT` is still 2.60 — so a bezel already in hand is dimensionally fine and only reads shallower than JP wants. **0.96 is terminal**: it leaves 2.04 mm over the glass against the ≥2.00 assert, so there is no seventh layer — deeper means revisiting the glass floor itself. The visible face is the bed face — use a smooth PEI sheet. Mic flare + window are bed-side chamfers, so they self-support. **The debossed honeycomb is a bed-side recess** (r6, 2026-08-26: the wyrm mark left the bezel — the four-edge tap ramp evicted it, JP's thumb-access verdict; the creature remains on the stand grille and the screen. The brow now carries honeycomb like the chin), 0.96 mm deep — exactly **six** layers at this part's 0.16 mm — and print as bridged voids, which is *why* they are recesses: on a bed face, relief only goes inward. Deeper again means each recess bridges over a 0.96 mm step instead of 0.80, so run the fan hard for the first few layers as you would on the shell's cap recesses. **2.04 mm of bezel is left over the glass** — measured on the exported mesh, not inferred: a 0.4 mm ray-cast grid found 3880 of 3880 debossed points sitting on a full-thickness 2.040 mm slab (re-measured at 0.96, 2026-08-25), so no region is thinned from the back. |
...
> **Mesh state: all four STLs are watertight** — zero boundary edges, zero non-manifold
> edges, verified by triangle arithmetic on the exported meshes (every undirected edge
> shared by exactly two triangles, every directed edge appearing exactly once).
> *(Retired 2026-08-26: this note used to disclose 3 non-manifold edges in
> `ember-front-bezel.stl`, coplanar-seam artefacts of the wyrm mark's 104 stacked
> row-spans. r6 removed the mark — the four-edge tap ramp evicted it — and a re-measure
> of the r6 export counts exactly 0. If your slicer reports non-manifold edges on a
> CURRENT bezel STL, that is now a real finding, not the known artefact.)*
...
⚠️ **At the r4 depth (0.96, #48) this stops being cosmetic.** The deboss now clears the ≥2.00 mm
glass-floor assert by only **0.04 mm** — and a 0.20 first layer shifts every boundary up by
exactly that 0.04. The same rounding that printed 0.80 as 0.84 would print 0.96 as **1.00**,
leaving **2.00 mm dead on the limit** with zero designed margin. The first-layer setting is now
load-bearing for the glass floor, not just for layer-boundary tidiness.
...
| `BEZEL_DEBOSS` | 0.96 | how deep the honeycomb and wyrm cut into the front face. **Keep it an exact multiple of the bezel's 0.16 mm layer height** — it was 0.45, which is 2.8125 layers, so the recess floor landed wherever the slicer's rounding fell rather than on a real layer boundary. **0.96 is 6 layers exactly.** It was 0.48 (3 layers) through the first print — too shallow to read at arm's length, #34 took it to 0.80 (5 layers), depth only, sizes untouched — and JP's verdict on the printed 0.80 part was "it's good but go even deeper next time pls" (#48). An assert holds ≥2.00 mm of bezel over the glass; at 0.96 the measured floor is **2.04 mm**, so **this is the last depth that fits and there is no room to keep going** — deeper means revisiting the glass floor, which is a different conversation. The shell runs `DEBOSS_BIG` at 0.80 on a thinner wall |
...
- **The bezel face is debossed too, and for the same reason on the opposite face.** 57
  ...
  0.70 mm web), and the hearth-wyrm **25.65 × 11.25 mm** in the brow — all **0.96 mm** deep,
  which is exactly six layers at this part's 0.16 mm (it was three, which #34 found too
  shallow to read on the printed part, then five, which JP validated but asked to go deeper
  still — #48; the sizes above are unchanged throughout). This part
